#ifndef __CAST_APP_HPP__
#define __CAST_APP_HPP__
#include <stdio.h>
...
void errorFn(const char* err) { ERROR << "error: " << err; }
...
void buttonFn(CusButton btn, int clicks) {
  PRINT << ((btn == ButtonDown) ? "down" : "up")
        << " button pressed, clicks: " << clicks;
}
...
/// @param[in] npos the # of positional data points embedded with the frame
/// @param[in] pos the buffer of positional data
void newRawImageFn(const void* newImage, const CusRawImageInfo* nfo, int npos,
                   const CusPosInfo* pos) {
  lasttime_ = nfo->tm;
#ifdef PRINTRAW
  if (nfo->rf)
    PRINT << "new rf data (" << newImage << "): " << nfo->lines << " x "
          << nfo->samples << " @ " << nfo->bitsPerSample << "bits. @ "
          << nfo->axialSize << " microns per sample. imu points: " << npos;
  else
    PRINT << "new pre-scan data (" << newImage << "): " << nfo->lines << " x "
          << nfo->samples << " @ " << nfo->bitsPerSample << "bits. @ "
          << nfo->axialSize << " microns per sample. imu points: " << npos
          << " jpeg size: " << static_cast<int>(nfo->jpeg);

  if (npos) printImuData(npos, pos);
#else
  (void)newImage;
  (void)npos;
  (void)pos;
...
/// @param[in] npos the # of positional data points embedded with the frame
/// @param[in] pos the buffer of positional data
void newProcessedImageFn(const void* newImage, const CusProcessedImageInfo* nfo,
  ...
  cv::Mat us_image(height, width, CV_8UC4, const_cast<void*>(newImage));
...
void newSpectralImageFn(const void* newImage, const CusSpectralImageInfo* nfo) {
  ...
  if (streamOutput_)
    PRINTSL << "new spectrum: " << nfo->lines << " x " << nfo->samples << " @ "
            << nfo->bitsPerSample << "bits. @ " << nfo->period << " sec/line."
            << std::flush;
}
...
bool saveRawData() {
  if (!szRawData_ || !buffer_) return false;
...
char getCommand(const std::string& line) {
  if (line.empty() || (line.size() > 1 && line[1] != ' ')) return '\0';
  return line[0];
}

std::vector<std::string> getParameters(const std::string& line,
                                       std::size_t max) {
  std::vector<std::string> result;
  if (line.size() < 2 || line[1] != ' ') return result;
  std::size_t lastIndex = 2;
  std::size_t nextIndex = line.find(' ', lastIndex);
  while (lastIndex < line.size()) {
    const bool addAll =
        (nextIndex == std::string::npos || result.size() + 1 == max);
...
void processEventLoop(std::atomic_bool& quit) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    if (cmd == 'Q' || cmd == 'q') {
      ...
      double x = 0;
      double y = 0;
      if (prms.size() < 3 || !parseDouble(x, prms[0]) ||
          !parseDouble(y, prms[1])) {
        ERROR << "wrong label parameters provided";
        ERROR << "please give parameters as -> x y text";
        ERROR << "where x and y are the coordinates of the center of the label"
              << std::endl;
        continue;
      }
      if (castAddLabelOverlay(captureID_, prms.back().c_str(), x, y, 100.0,
                                 100.0) < 0)
        ERROR << "failed to add label to capture" << std::endl;
      else
        PRINT << "added label '" << prms.back() << "' at (" << x << ", " << y
              << ") to capture" << std::endl;
    } else if (cmd == 'm' || cmd == 'M') {
      if (captureID_ < 0) {
        ERROR << "no capture in progress" << std::endl;
      ...
      double x2 = 0;
      double y2 = 0;
      if (prms.size() < 5 || !parseDouble(x1, prms[0]) ||
          !parseDouble(y1, prms[1]) || !parseDouble(x2, prms[2]) ||
          !parseDouble(y2, prms[3])) {
        ERROR << "wrong measurement parameters provided";
        ERROR << "please give parameters as -> x1 y1 x2 y2 text";
        ERROR << "for a measurement from (x1,y1) to (x2,y2) with label 'text'"
              << std::endl;
        continue;
      }
      const double points[] = {x1, y1, x2, y2};
      const int nDoubles = static_cast<int>(sizeof(points) / sizeof(points[0]));
      if (castAddMeasurement(captureID_, CusMeasurementTypeDistance,
                                prms.back().c_str(), points, nDoubles) < 0)
        ERROR << "failed to add label to capture" << std::endl;
      else
        PRINT << "added measurement '" << prms.back() << "' from (" << x1
              << ", " << y1 << ") "
      ...
      if (prms[1] == "true" || prms[1] == "false") {
        if (castEnableParameter(
                prms[0].c_str(), (prms[1] == "true"), [](int ret) {
                  if (ret == CUS_FAILURE)
        ...
        if (castSetParameter(prms[0].c_str(), val, [](int ret) {
              if (ret == CUS_FAILURE) ERROR << "parameter setting parameter";
            }) < 0) {
      ...
      PRINT << "       capture: [c: start/end capture, l: add label, m: add "
               "measurement]"
            << std::endl;
    }
  }
...
int init(int& argc, char** argv) {
  ...
  // Windows: Visual C++ doesn't have 'getopt' so use Boost's program_options
  // instead
#ifdef _MSC_VER
  ...
  try {
    ...
    desc.add_options()("help", "produce help message")(
        "address", po::value<std::string>(&ipAddr)->required(),
        "set the IP address of the host scanner")(
        "port", po::value<unsigned int>(&port)->required(),
        "set the port of the host scanner")(
        "keydir", po::value<std::string>(&keydir)->default_value("/tmp/"),
        "set the path containing the security keys");
...
#else  // every other platform has 'getopt' which we're using so as to not pull
       // in the Boost dependency
  int o;
  keydir = "/tmp/";
  ...
  while ((o = getopt(argc, argv, "k:a:p:")) != -1) {
    switch (o) {
      ...
      // invalid argument
      case '?':
        PRINT << "invalid argument, valid options: -a [addr], -p [port], -k "
                 "[keydir]";
        break;
      default:
        break;
                     ...
                     [](int imagePort, int imuPort, int swRevMatch) {
                       if (imagePort == CUS_FAILURE)
                         ...
                         if (swRevMatch == CUS_FAILURE)
                           ERROR << "software revisions do not match, that is "
